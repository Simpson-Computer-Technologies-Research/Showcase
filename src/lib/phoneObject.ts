...
// Load the 3D Object
const GLTF_LOADER: GLTFLoader = new GLTFLoader(
...
export const LoadPhoneObjectWithGSAP = async (Scene, Wallpaper) =>
  GLTF_LOADER.load("./iphone.glb", async (model) => {
    ...
    // Phone Rotation
    GSAP.timeline({ scrollTrigger: { scrub: 0, ease: "expo" } }).to(model.rotation, {
			x: Math.PI * 3.9,
			onUpdate: () => {
				// Scroll Percentage
				const SCROLL_PERCENTAGE = (window.scrollY / (document.body.offsetHeight - window.innerHeight)) * 100;
