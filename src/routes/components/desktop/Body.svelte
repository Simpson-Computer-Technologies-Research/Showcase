...
<div class="fixed z-20 h-4 bottom-0 bg-gray-50 w-screen">
	<div
		class="fixed h-4 bottom-0 bg-[#000000]"
...
<section class="section mt-20 sticky top-32">
	...
	{#if ScrollPercentage < 50 && ScrollPercentage > 20}
		...
		>
			Engage with <mark
				class="text-[#35d0ff] font-black"
				style="background: none;">Polls</mark
			>
			and
			<mark class="text-[#35d0ff] font-black" style="background: none;"
				>Local News</mark
			>
		</h2>
	...
	{:else if ScrollPercentage > 65}
		...
		>
			Meetups made <mark
				class="text-[#35d0ff] font-black"
				style="background: none;">easy.</mark
			>
		</h2>
	{/if}
...
<!-- Maps Divider -->
<div style="height: {window.innerHeight + 900}px;" />
